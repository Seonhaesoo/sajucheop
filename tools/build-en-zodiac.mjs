...
const NAV = (rel) => [{ href: rel + 'en/', label: 'Chart' }, { href: rel + 'en/zodiac/', label: 'Zodiac' }, { href: rel + 'en/2027/', label: '2027' }, { href: rel + 'en/match/', label: 'Match' }, { href: rel + 'en/guide/', label: 'Library' }, { href: rel, label: '한국어' }];
const STYLE = `<style>
...
const pl = (n) => (/Ox$/.test(n) ? n + 'en' : n + 's');   /* Ox → Oxen */
const an = (w) => (/^[AEIOU]/.test(w) ? 'an ' : 'a ') + w;
/* 설명문은 155자 안에서 붙일 수 있는 꼬리까지만 */
const fitDesc = (base, tails) => tails.map((t) => base + t).find((s) => s.length <= 155) || base;
...
function pairPage(a, b) {
  ...
  const map = { A: A.name, As: pl(A.name), B: B.name, ah: han(a), bh: han(b), ae: elA.en, be: elB.en, metaphor, group: p === 'samhap' ? SAMHAP_EN[SAMHAP_G[a]] : p === 'banghap' ? BANGHAP_EN[BANGHAP_G[a]] : '' };
  ...
  const desc = same
    ? fitDesc(`Two ${pl(A.name)} score ${score}/100 in Chinese zodiac compatibility (${R.label.toLowerCase()}).`, [' Love, friendship, family and how to make it work.', ' Love, friendship and family.'])
    : fitDesc(`${A.name} and ${B.name} compatibility: ${score}/100, ${g.label.toLowerCase()}. ${branchLinks ? branchLinks + ' between their branches, ' : 'No fixed branch link, '}${elA.en} and ${elB.en} ${er === 'same' ? 'as the same element' : er === 'gen' || er === 'gen_by' ? 'in a nourishing cycle' : 'in a controlling cycle'}.`,
      [` Love, friendship and family, also read as ${B.name} and ${A.name}.`, ' Love, friendship and family.']);
...
function compatIndex() {
  ...
  const title = 'Chinese Zodiac Compatibility Chart — All 78 Pairings Scored';
  const desc = 'Every Chinese zodiac pairing scored out of 100: secret friends, trines, clashes and harms between the twelve animals, with a reading for each pair.';
  const body = `
  <article class="guide-article">
    ...
    <div class="ga-body">
      ...
      <p>Korean couples go further than the year animal: <a href="${rel}en/guide/gunghap/">gunghap</a> compares two full birth charts, starting from the Day Masters. Try it with two birth dates on the <a href="${rel}en/match/">match page</a>.</p>
...
function animalPage(b) {
  ...
  <article class="guide-article">
    ...
    <div class="ga-body">
      ...
      <h2>The five ${pl(A.name)}</h2>
      <p>The year’s heavenly stem adds one of five elements, so a ${ys.find((x) => x.y >= 1980).y} ${A.name} and a ${ys.find((x) => x.y >= 1980).y + 12} one are not quite the same animal.</p>
...
function yearPage(y) {
  ...
  const sajuNote = cnyBeforeIp
    ? `A birthday from ${fmtMD(gapFrom)} to ${fmtMD(gapTo)}, ${y} is already ${an(A.name)} by Lunar New Year but still ${prev.han} (${prev.name}) in a saju chart.`
    : `A birthday from ${fmtMD(gapFrom)}${gapOne ? '' : ` to ${fmtMD(gapTo)}`}, ${y} is already ${x.han} (${x.name}) in a saju chart but still ${an(prev.A.name)} by Lunar New Year.`;
  ...
  <article class="guide-article">
    ...
    <div class="ga-body">
      ...
      <p>${y <= NOW ? `People born in the ${y} ${A.name} year turn ${age(NOW)} in ${NOW} (${age(NOW) + 1} in <a href="${rel}en/korean-age/">Korean age</a>) and ${age(NOW + 1)} in ${NOW + 1}. ` : `Children born from ${fmtD(x.span.start)} to ${fmtD(x.span.end)} will be ${pl(x.name)}. `}The same ${x.name} returns every sixty years${same60.length ? ` — ${same60.join(' and ')}` : ''}.</p>
      <h2>${A.name} years</h2>
      <p>${sameSign}</p>
      <h2>The ${A.name} in 2027</h2>
...
function hub() {
  ...
  const title = 'Chinese Zodiac Calculator: What Is My Animal Sign?';
  const desc = 'Find your Chinese zodiac animal and element from your birth date, using the real Lunar New Year boundary, not January 1. Years 1924–2031 and every sign.';
  const body = `
  <article class="guide-article">
    ...
    <div class="ga-body">
      ...
      <p class="callout">The zodiac animal is one pillar of four. Your month, day and hour of birth complete a <a href="${rel}en/">Korean Four Pillars (saju) chart</a> — the Day Master in the <a href="${rel}en/guide/">library</a> describes you more precisely than the year animal does. More calendar pages: the <a href="${rel}en/korean-age/">Korean age calculator</a>, the <a href="${rel}en/lunar-birthday/">lunar birthday calculator</a> and <a href="${rel}en/lunar-new-year/">Lunar New Year dates</a>.</p>
    </div>
  </article>
  <script>
  (function () {
