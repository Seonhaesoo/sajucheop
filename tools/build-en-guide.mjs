...
const PUBLISHED = '2026-09-05', MODIFIED = '2026-09-13';
const NAV = (rel) => [{ href: rel + 'en/', label: 'Chart' }, { href: rel + 'en/today/', label: 'Today' }, { href: rel + 'en/zodiac/', label: 'Zodiac' }, { href: rel + 'en/match/', label: 'Match' }, { href: rel + 'en/guide/', label: 'Library' }, { href: rel, label: '한국어' }];
const STYLE = `<style>
    ...
    .lr-list { display: flex; flex-direction: column; gap: 8px; margin: 8px 0 18px; }
    .lr-list a { display: block; padding: 12px 14px; background: #FFFDF9; border: 1px solid var(--line-soft); border-radius: 10px; text-decoration: none; color: inherit; }
    .lr-list a:hover { border-color: var(--seal); }
    .lr-list a b { display: block; font-family: 'Noto Serif KR', serif; font-size: 15.5px; line-height: 1.4; }
    .lr-list a small { display: block; font-size: 12.5px; color: var(--muted); margin-top: 4px; line-height: 1.55; }
  </style>`;
...
DAY_MASTERS.forEach((d, i) => {
  ...
  const title = `${d.name} Day Master (${d.han}) — ${d.arch} | Saju`;
  const desc = `What a ${d.name} (${d.han}) Day Master is like in Korean saju: ${d.arch.replace(/^The /, 'the ')}’s strengths, blind spots, love, work, and who they combine and clash with.`;
  ...
  <article class="guide-article">
    ...
    <div class="ga-body">
      ...
      <p>The same Day Master reads differently depending on how well it is supported by the other seven characters. <a href="${rel}en/guide/day-master-strength/">How strength is judged →</a></p>
      <ul class="dm-list">
  ...
  <link rel="alternate" hreflang="ja" href="${SITE}/ja/nikkan/${d.slug}/">`,
...
/* Day Master hub — /en/guide/day-master/ (the folder had no index page, and “day master” is a search term of its own) */
{
  ...
  <link rel="alternate" hreflang="ja" href="${SITE}/ja/nikkan/">`,
...
/* Index */
{
  const rel = '../../';
  const cards = DAY_MASTERS.map((d) => `<a href="${rel}en/guide/day-master/${d.slug}/"><i>${d.han}</i><span><b>${d.arch}</b><small>${d.name} · ${esc(d.tagline)}</small></span></a>`).join('\n        ');
  const title = 'Saju Library — Korean Four Pillars in Plain English';
  const desc = 'Korean saju explained in English: the ten Day Masters, Five Elements, Ten Gods, luck pillars, compatibility and saju in everyday Korean life.';
  const sections = CATS.map((c) => {
    const list = ARTICLES.filter((a) => a.cat === c.key);
    return list.length ? `<h2 id="${c.key}">${esc(c.label)}</h2>
      <p>${esc(c.note)}</p>
      <nav class="lr-list">
        ${list.map((a) => `<a href="${rel}en/guide/${a.slug}/"><b>${esc(a.title)}</b><small>${esc(a.desc)}</small></a>`).join('\n        ')}
      </nav>` : '';
  }).join('\n      ');
  const body = `
  <article class="guide-article">
    <div class="ga-overline">Library</div>
    <h1 class="ga-title">The saju library — <br>Korean Four Pillars in plain English</h1>
    <p class="ga-meta">Sajucheop library · ${ARTICLES.length} articles · 10 Day Masters · 60 Day Pillars · 100 pairings</p>
    <p class="ga-lead">Saju reads a birth date as eight characters and the relationships between them. This library explains how, without the jargon. Start with <a href="${rel}en/guide/what-is-saju/">what saju is</a>, find your nature among the ten Day Masters below, or <a href="${rel}en/">enter your birth date</a> and read your own chart alongside.</p>
    <div class="ga-body">
      ${sections}
      ...
      <p>Then the reading tools themselves: the <a href="${rel}en/guide/ten-gods/">Ten Gods, one page each</a>, the <a href="${rel}en/guide/twelve-stages/">Twelve Life Stages</a> your Day Master moves through, the <a href="${rel}en/guide/#stars">special stars</a>, and a <a href="${rel}en/glossary/">glossary of about a hundred terms</a>. For 2027, read the year by <a href="${rel}en/2027/day-master/">Day Master</a> or by <a href="${rel}en/2027/day-pillar/">day pillar</a>, and try the <a href="${rel}en/bazi-calculator/">BaZi calculator</a> if you prefer Chinese terminology.</p>
      <p class="callout"><a href="${rel}en/today/">Today\'s energy</a> · <a href="${rel}en/match/">Compatibility</a> · <a href="${rel}en/zodiac/">Chinese zodiac</a> · <a href="${rel}en/korean-age/">Korean age</a> · <a href="${rel}en/lunar-birthday/">Lunar birthday</a> · <a href="${rel}guide/" hreflang="ko">한국어 서재</a></p>
    </div>
    <div class="ga-cta">
      <a class="btn-primary" href="${rel}en/"><span class="seal-dot" aria-hidden="true"></span><span>Find my Day Master</span></a>
    </div>
  </article>`;
  write('en/guide', shell({ rel, lang: 'en', title, desc, canonical: SITE + '/en/guide/', nav: NAV(rel), ogTitle: 'Saju Library — Korean Four Pillars in plain English', extraHead: STYLE + `\n  <link rel="alternate" hreflang="en" href="${SITE}/en/guide/">\n  <link rel="alternate" hreflang="ko" href="${SITE}/guide/">`,
