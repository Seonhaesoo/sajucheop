...
const REL = {
  hap: { label: '찰떡 조합', tag: '천간합',
    first: '열 개의 천간 가운데 서로 맞물리는 단 하나의 짝입니다. 처음 만나도 낯설지 않고, 함께 있으면 없던 힘이 생겨요. 설명하기 어려운 끌림이 있다면 그게 합(合)의 감각입니다.',
  ...
  chung: { label: '긴장 조합', tag: '천간충',
    ...
    advice: '상대를 바꾸려는 마음을 내려놓는 순간 관계가 열립니다. 충은 이기는 관계가 아니라 배우는 관계예요.' },
  bihwa: { label: '편안한 조합', tag: '비화',
    first: '같은 기운끼리 만났습니다. 서로를 가장 잘 알아보는 동료 같은 사이 — 말이 통하고 속도가 비슷해 금세 가까워져요. 대신 닮은 단점까지 똑같아서, 양보가 없으면 같은 지점에서 부딪힙니다.',
...
function relOf(a, b) {
  ...
  if (STEM_HAP[a] === b) {
    ...
    return { key: 'hap', title: '천간의 합(合) — 서로에게 끌리는 조합', body: `${sa.kor}${sa.el}·${sb.kor}${sb.el} 두 천간은 열 개 가운데 서로 맞물리는 단 하나의 짝입니다. 두 기운이 만나 ${hwa}(${EL_HAN[hwa]})의 기운으로 화(化)해, 함께 있을 때 없던 힘이 생겨요.` };
  }
  if (STEM_CHUNG[a] === b) return { key: 'chung', title: '천간의 충(沖) — 강하게 부딪히는 조합', body: `${sa.kor}${sa.el}·${sb.kor}${sb.el} 두 천간은 정면으로 마주 서는 기운입니다. 밀어내는 만큼 서로를 성장시키는 자극이 되기도 해요. 거리와 예의가 이 관계의 기술입니다.` };
...
for (let a = 0; a < 10; a++) {
  ...
  <article class="guide-article">
    ...
    <p class="ga-meta">${josa(`${esc(ca.metaphor)}(${A})이 ${esc(cb.metaphor)}(${B})을`)} 만나면</p>
